...
static void joyEvent_Demo(u16 joy, u16 changed, u16 state);
static void mainLoop_Demo(void);
static void endLoop_Demo(void);
...
static fix32 camposx;
static fix32 camposy;
static fix32 posx;
static fix32 posy;
static fix32 movx;
static fix32 movy;
...
static fix32 enemyPosx[2];
static fix32 enemyPosy[2];
static s16 enemyXorder[2];
...
//	SND_setPCM_XGM(SFX_JUMP, se010_sword_sfx, sizeof(se010_sword_sfx));
//	SND_setPCM_XGM(SFX_JUMP, v0002_sfx, sizeof(v0002_sfx));
...
//	sprites[1] = SPR_addSprite(&enemies_sprite, fix32ToInt(enemyPosx[0] - camposx), fix32ToInt(enemyPosy[0] - camposy), TILE_ATTR(PAL3, TRUE, FALSE, FALSE));
	sprites[1] = SPR_addSprite(&zako1_sprite, fix32ToInt(enemyPosx[0] - camposx), fix32ToInt(enemyPosy[0] - camposy), TILE_ATTR(PAL3, TRUE, FALSE, FALSE));
//	sprites[2] = SPR_addSprite(&enemies_sprite, fix32ToInt(enemyPosx[1] - camposx), fix32ToInt(enemyPosy[1] - camposy), TILE_ATTR(PAL3, TRUE, FALSE, FALSE));
	sprites[2] = SPR_addSprite(&my1_sprite, fix32ToInt(enemyPosx[1] - camposx), fix32ToInt(enemyPosy[1] - camposy), TILE_ATTR(PAL3, TRUE, FALSE, FALSE));
...
//	memcpy(&palette[48], enemies_sprite.palette->data, 16 * 2);
	memcpy(&palette[48], zako1_sprite.palette->data, 16 * 2);
	// fade in
//	VDP_fadeIn(0, (4 * 16) - 1, palette, 20, FALSE);
	VDP_setPaletteColors(0,palette,4*16);
	//
	JOY_setEventHandler(joyEvent_Demo);
	g_mainloop_callback=&mainLoop_Demo;
}
static void endLoop_Demo(void){
	XGM_stopPlay();
	returnMenu();
...
static void updateAnim(){
	// jumping
	if (movy){
		SPR_setAnim(sprites[0], ANIM_ROLL);
	}else{
